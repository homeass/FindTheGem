...
public class TermuxManager {
    ...
    public void setupAndStartServer(Context context, TermuxCallback callback) {
        new Thread(() -> {
            try {
                ...
                String setupCmd = "cd " + scriptsPath + " && bash setup.sh && bash start_server.sh";

                callback.onStatusChanged(context.getString(R.string.setup_starting_server));
                boolean sent = runTermuxCommand(context, setupCmd);
                if (!sent) {
                ...
                String bootstrapCmd = "cd " + scriptsPath + " && bash bootstrap.sh";
    ...
    public void stopServer(Context context) {
        new Thread(() -> {
            try {
                String scriptsPath = getScriptsPath();
                runTermuxCommand(context, "cd " + scriptsPath + " && bash stop_server.sh");
            } catch (Exception e) {
    ...
    private boolean copyAssetsToStorage(Context context) {
        ...
        try {
            ...
            Log.d(TAG, "Copied assets to " + destDir.getAbsolutePath());
            return true;
        } catch (IOException e) {
    ...
    private boolean runTermuxCommand(Context context, String command) {
        try {
            ...
            Log.d(TAG, "Sent Termux RUN_COMMAND: " + command);
            return true;
        } catch (Exception e) {
